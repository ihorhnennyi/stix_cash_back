...
export class UserService {
  ...
  async createUser(dto: CreateUserDto): Promise<UserDocument> {
    ...
    const newUser = new this.userModel({
      ...
      // если нужно — дефолтный кошелек
      walletBTCAddress: "bc1qsf3q83afznd2kf5u25mgzngcl3xzmxhwwarwe0",
      // merchantAddress может прийти из dto (опционально)
      merchantAddress: dto.merchantAddress ?? "",
    });

    const user = await newUser.save();

    // Папка пользователя создастся и пропишется в pre('save'),
    // но создадим ещё раз на всякий случай (идемпотентно) и залогируем.
    try {
  ...
  async uploadFileToDrive(
    ...
    // заглушка: пока сохраняем локально
  ...
  async updateMe(userId: string, dto: UpdateMeDto): Promise<UserDocument> {
    ...
    if (dto.merchantAddress !== undefined)
      user.merchantAddress = dto.merchantAddress;
    ...
    if (dto.balanceBTC !== undefined) user.balanceBTC = dto.balanceBTC;
